...
class MonopolyEnv:
    ...
    def _color_group_progress(self, pid: int) -> float:
        """
        Best squared ownership-fraction across every color group (including
        railroads/utilities) — the group we're closest to completing dominates,
        rather than being diluted by the other 9 groups we may own nothing in.
        Squaring makes near-complete groups worth much more than scattered
        partial ownership. Independent formula from the ASU teacher's
        discounted-monopoly-value calculation — deliberately simpler (no dice
        projection, no development search).
        """
        best = 0.0
        for squares in COLOR_GROUPS.values():
            owned = sum(1 for s in squares if self.properties[s].owner == pid)
            best = max(best, (owned / len(squares)) ** 2)
        return best

    def _house_threshold_score(self, pid: int) -> float:
        """
        Real Monopoly economics: rent jumps sharply at 3 houses per property,
        so reaching 3 houses on a fully-owned color group matters far more
        than a 4th house or a hotel. Rewards the closest-to-3-houses
        fully-owned group; 0.0 if no color group is fully owned by pid.
        """
        best = 0.0
        for squares in COLOR_GROUPS.values():
            if any(self.properties[s].owner != pid for s in squares):
                continue
            avg_houses = sum(self.properties[s].houses for s in squares) / len(
                squares
            )
            best = max(best, min(avg_houses, 3.0) / 3.0)
        return best
    ...
    def _compute_reward(self, pid: int) -> float:
        ...
        house_term = self._relative_score(
            self._house_threshold_score(pid),
            [
                self._house_threshold_score(p.player_id)
                for p in active
                if p.player_id != pid
            ],
        )
