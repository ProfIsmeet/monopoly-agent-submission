...
from .actions import ActionType, OFFSETS, action_to_description
from .agents_fixed import FixedPolicyAgent, FPAgentA, FPAgentB, FPAgentC, FP_AGENT_CLASSES
from .constants import NUM_PLAYERS, PROPERTY_IDS
from .env import MonopolyEnv

POTENTIAL_REWARD_LIMIT = 2.0

# Flat penalty subtracted from the shaped reward whenever the learning agent
# mortgages or unmortgages a property. Bounded PBRS (Fix 5, POTENTIAL_REWARD_LIMIT
# clipping) is not perfectly telescoping -- clipping the potential can leave a
# small net-positive residual across a mortgage->unmortgage round trip, which a
# policy can farm indefinitely without any real economic progress (observed in
# the 800-game probe: the learning agent spent ~8% of its actions flipping the
# same property's mortgage state back and forth within a handful of steps).
# This flat, unclipped tax makes each individual mortgage/unmortgage action
# reward-negative on its own, closing that loop regardless of clipping edge
# cases.
MORTGAGE_CYCLE_PENALTY = 0.03


def _is_mortgage_action(action: int) -> bool:
    return OFFSETS["mortgage"] <= action < OFFSETS["mortgage"] + len(PROPERTY_IDS)


def _is_unmortgage_action(action: int) -> bool:
    return OFFSETS["unmortgage"] <= action < OFFSETS["unmortgage"] + len(PROPERTY_IDS)
...
def run_episode(
    ...
    If action_log_file is given, every action taken by every player (learning
    agent and fixed-policy opponents alike) is appended as one JSON line —
    the full move-by-move match transcript, not just the learning agent's
    per-game summary.
    """

    def log_action(pid: int, action: int, local_step: int) -> None:
            ...
            json.dumps(
                {
                    ...
                    "step": local_step,
                    "player_id": pid,
                    "is_learning_agent": pid == agent_pid,
                    "action": action,
    ...
    def potential_delta(start: float, terminal: bool = False) -> float:
        # _compute_reward already returns the correct terminal potential
        # (+1.0 on elimination win, -1.0 on own bankruptcy) via its own
        # active-player/bankrupt checks, so it is safe to call unconditionally
        # here too — forcing it to 0.0 at terminal previously penalized
        # winning trajectories (which reach terminal with high Phi) and
        # rewarded losing ones (which reach terminal with low/negative Phi).
        next_potential = env._compute_reward(agent_pid)
        ...
        if pid == agent_pid:
            ...
            if is_ppo:
                ...
                if log_prob is not None and pending_transition is not None:
                    ...
                    prev_action = pending_transition[1]
                    if _is_mortgage_action(prev_action) or _is_unmortgage_action(
                        prev_action
                    ):
                        reward -= MORTGAGE_CYCLE_PENALTY
                    total_reward += reward
                    if update_online:
            ...
            log_action(pid, action, step_count)
    ...
    if pending_transition is not None:
        ...
        prev_action = pending_transition[1]
        if _is_mortgage_action(prev_action) or _is_unmortgage_action(prev_action):
            reward -= MORTGAGE_CYCLE_PENALTY
        total_reward += reward
...
def train(
    ...
    If game_log_path is given, one JSON line per completed game is appended
    to it (flushed immediately), independent of log_every, so every game's
    result survives even if training is interrupted mid-run.

    If action_log_path is given, one JSON line per individual action taken
    by ANY player (learning agent and fixed-policy opponents alike) is
    appended to it — the full move-by-move match transcript.
    """
    random.seed(seed)
    np.random.seed(seed)
    ...
    if game_log_path is not None:
        Path(game_log_path).parent.mkdir(parents=True, exist_ok=True)
        game_log_file = open(game_log_path, "a" if resume else "w")

    action_log_file = None
    if action_log_path is not None:
        Path(action_log_path).parent.mkdir(parents=True, exist_ok=True)
        action_log_file = open(action_log_path, "a" if resume else "w")

    stats_log_file = None
    if stats_log_path is not None:
        Path(stats_log_path).parent.mkdir(parents=True, exist_ok=True)
        stats_log_file = open(stats_log_path, "a" if resume else "w")
    ...
    for game_num in range(1, n_games + 1):
        ...
        if opponent_pool == "rival_mix":
            # One verified competitor-team heuristic (EXPO) plus two of our
            # own fixed personalities, filling the other two seats. EXPO was
            # confirmed independent of any organizer-shared reference module
            # before inclusion (see rival_agents/ source comments) and shares
            # our exact action-ID space (its actions.py is byte-identical to
            # ours — same shared course template). A second candidate was
            # dropped: its original repo named the class ASU_SLAYER, and
            # renaming it was judged too close to the banned ASU association
            # to risk, regardless of the algorithm's actual independence.
            from .rival_pool import RIVAL_AGENT_CLASSES

            fp_agents = [
                RIVAL_AGENT_CLASSES[0](other_pids[0]),
                FPAgentB(other_pids[1]),
                FPAgentC(other_pids[2]),
            ]
        else:
            # Sample 3 of the 6 fixed-policy personalities per game (not a
            # static A/B/C trio) — the real tournament pits us against other
            # teams' agents, so training against one fixed personality mix
            # risks overfitting to their specific accept/reject quirks
            # (verified: all 6 personalities have narrow, mutually different
            # trade-acceptance rules in agents_fixed.py). A broader mix during
            # training is cheap insurance against that overfit, per
            # ARCHITECTURE.md P1 #10.
            fp_classes = random.sample(FP_AGENT_CLASSES, 3)
        ...
        result = run_episode(
            ...
            game_num=absolute_game,
        )
        games_completed = game_num
        learning_agent.games_trained = absolute_game

        if stats_log_file is not None and result["stats"]:
            stats_log_file.write(
                json.dumps({"game": absolute_game, **result["stats"]}) + "\n"
            )
                ...
                json.dumps(
                    {
                        ...
                        "steps": result["steps"],
                        "trades_initiated": result["trades_initiated"],
                        "trades_accepted": result["trades_accepted"],
                        "trades_declined": result["trades_declined"],
                        "properties_acquired": result["properties_acquired"],
                    }
                )
                + "\n"
            )
        ...
        window_entropy += result["stats"].get("entropy", 0.0) if result["stats"] else 0.0
        ...
        if game_num % log_every == 0:
            ...
            avg_entropy = window_entropy / window_games
            ...
            # Policy entropy falling toward 0 means the actor is collapsing
            # onto a near-deterministic policy (converged, or stuck) — a
            # flat/rising entropy alongside a flat win-rate means it's still
            # exploring without learning. Watch the trend, not one window.
            entropy_str = f"  H={avg_entropy:.3f}" if is_ppo else ""
            print(
                f"  Game {absolute_game:5d} | "
                f"Win%: {win_rate:5.1f}%{eps_str}{entropy_str} | "
