...
class ActorNetwork(nn.Module):
    ...
    def get_action(self, state: np.ndarray, allowed_actions: list, generator=None):
        """
        Sample an action given allowed actions.

        `generator` should be a torch.Generator private to the caller (not
        the global default) — tournament harnesses pair identical global RNG
        seeds across all four seats, and sampling from the default generator
        here would desynchronize that pairing. Omit it to keep the old
        (global-RNG) behavior for callers that don't care, e.g. training.
        """
        ...
        multinomial_kwargs = {"generator": generator} if generator is not None else {}
        action = torch.multinomial(probs, 1, **multinomial_kwargs).item()
