...
distil the reference teacher module, and no the reference solver output was used as a training label.
The two differ structurally:

  * the reference solver scores M_assets + R_short + R_long + M_monopoly, pricing deeds at
    ...
    with cash at 1.0x.
  * the reference solver clones the environment for every legal action and evaluates the
    resulting state. EXPO never clones and never rolls out; it computes
    closed-form book and rent deltas per action family.
  * the reference solver's long-run rent term is 5 laps x 40/7 landings uniform over 28 deeds.
    ...
    Go-To-Jail teleport.
  * Cost: the reference solver ~894 ms/decision, EXPO ~0.07 ms/decision -- a copy cannot be
...
MEASURED (seat-balanced, paired seeds, ruleset ppo-plus-v2)
    ...
    vs the reference solver + Fixed-B/C       44 games   52.3%   (the reference solver 31.8%)
    ...
    def _best_trade_offer(self, env, allowed: List[int]):
        ...
        for action in allowed:
            ...
            if _section(action, "buy_trade"):
                ...
                # theirs by 1.6x) cost 11.4 points against the reference solver -- 52.3% -> 40.9%
                # over 44 paired games, while the reference solver rose 31.8% -> 54.5%.
