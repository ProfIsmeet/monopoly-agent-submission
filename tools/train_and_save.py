...
def main():
    ...
    parser.add_argument(
        "--rival-mix",
        action="store_true",
        help="Train against one verified competitor-team agent (EXPO) plus two "
        "fixed personalities, instead of the diverse 6-personality sample (PPO only)",
    )
    parser.add_argument(
        "--entropy-coef",
        type=float,
        default=None,
        help="On --resume, override the checkpoint's saved entropy_coef instead "
        "of inheriting it. Use when switching to a harder/narrower opponent "
        "pool (e.g. --rival-mix) so the policy keeps an exploration budget "
        "instead of collapsing into whatever the source run had converged to.",
    )
    parser.add_argument(
        "--fresh-optimizer",
        action="store_true",
        help="On --resume, reinitialize the Adam optimizer instead of restoring "
        "its saved state. Use alongside --entropy-coef when switching opponent "
        "pools, so stale momentum tuned to the old pool's gradients doesn't "
        "suppress adaptation to the new one.",
    )
    ...
    game_log_path = str(Path(args.out).with_suffix("")) + "_games.jsonl"
    action_log_path = str(Path(args.out).with_suffix("")) + "_actions.jsonl"
    stats_log_path = str(Path(args.out).with_suffix("")) + "_train_stats.jsonl"
    ...
    if args.algo == "ppo":
        agent, history = train_ppo(
            ...
            opponent_pool="rival_mix" if args.rival_mix else "diverse",
            entropy_coef_override=args.entropy_coef,
